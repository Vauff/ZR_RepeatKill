...
public Action ToggleRepeatKill(int client, int args)
{
	if (g_bBlockRespawn)
	{
		PrintToChatAll(" \x04[ZR]\x05 Repeat killer detector force toggled off. Re-enabling respawn for this round.");
		ServerCommand("zr_zspawn_force @all 1");
		g_bBlockRespawn = false;
	}
	else
	{
		PrintToChat(client, " \x04[ZR] \x05Repeat killer is already turned off!");
	}
...
public Action Event_PlayerDeath(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (victim && !attacker && StrEqual(weapon, "trigger_hurt"))
	{
		...
		{
			PrintToChatAll(" \x04[ZR]\x05 Repeat killer detected. Disabling respawn for this round.");
